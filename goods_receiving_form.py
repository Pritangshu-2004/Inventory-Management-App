...
from models import Product, GoodsReceiving
...
class GoodsReceivingForm(QWidget):
    ...
    def setup_ui(self):
        ...
        self.supplier = QLineEdit(); f.addRow("Supplier:", self.supplier)
    ...
    def save(self):
        try:
            ...
            total = quantity * rate
            GoodsReceiving.create(product=pid, supplier=self.supplier.text(),
                                  quantity=quantity, unit=self.unit.text(),
                                  rate=rate, total_rate=total, tax=tax)
            QMessageBox.information(self, "Saved", "Goods Received Recorded")
